...
SvtxEvaluator::SvtxEvaluator(const string &name, const string &filename,
		const string &trackmapname, unsigned int nlayers_maps,
		unsigned int nlayers_intt, unsigned int nlayers_tpc) :
		SubsysReco("SvtxEvaluator"), _ievent(0), _svtxevalstack(nullptr), _strict(
				false), _errors(0), _do_vertex_eval(true), _do_gpoint_eval(true), _do_g4hit_eval(
				true), _do_hit_eval(true), _do_cluster_eval(true), _do_gtrack_eval(
				true), _do_track_eval(true), _do_gseed_eval(false), _do_track_match(
				true), _do_eval_light(true), _scan_for_embedded(false), _nlayers_maps(
				nlayers_maps), _nlayers_intt(nlayers_intt), _nlayers_tpc(nlayers_tpc), _ntp_vertex(
				nullptr), _ntp_gpoint(nullptr), _ntp_g4hit(nullptr), _ntp_hit(nullptr), _ntp_cluster(
				nullptr), _ntp_gtrack(nullptr), _ntp_track(nullptr), _ntp_gseed(
				nullptr), _filename(filename), _trackmapname(trackmapname), _tfile(
				nullptr), _timer(nullptr) {
	verbosity = 0;
}
...
int SvtxEvaluator::Init(PHCompositeNode *topNode) {
	...
	if (_do_gtrack_eval)
		_ntp_gtrack =
				new TNtuple("ntp_gtrack", "g4particle => best svtxtrack",
	...
	if (_do_track_eval)
		_ntp_track =
				new TNtuple("ntp_track", "svtxtrack => max truth",
...
int SvtxEvaluator::End(PHCompositeNode *topNode) {
	...
	if (verbosity > 0) {
		cout
				<< "========================= SvtxEvaluator::End() ============================"
				<< endl;
		cout << " " << _ievent << " events of output written to: " << _filename
				<< endl;
		cout
				<< "==========================================================================="
				<< endl;
...
void SvtxEvaluator::printInputInfo(PHCompositeNode *topNode) {
	...
	if (verbosity > 1)
		cout << "SvtxEvaluator::printInputInfo() entered" << endl;
	...
	if (verbosity > 3) {
		...
		std::set<PHG4Hit*> g4hits =
				_svtxevalstack->get_truth_eval()->all_truth_hits();
		unsigned int ig4hit = 0;
		for (std::set<PHG4Hit*>::iterator iter = g4hits.begin();
				iter != g4hits.end(); ++iter) {
			PHG4Hit *g4hit = *iter;
...
void SvtxEvaluator::printOutputInfo(PHCompositeNode *topNode) {
	...
	if (verbosity > 1)
		cout << "SvtxEvaluator::printOutputInfo() entered" << endl;
	...
	if (verbosity > 0) {
		...
		PHG4VtxPoint *gvertex = truthinfo->GetPrimaryVtx(
				truthinfo->GetPrimaryVertexIndex());
		...
		if (hitmap) {
			for (SvtxHitMap::Iter iter = hitmap->begin(); iter != hitmap->end();
					++iter) {
		...
		for (unsigned int ilayer = 0;
				ilayer < _nlayers_maps + _nlayers_intt + _nlayers_tpc; ++ilayer) {
			cout << "layer " << ilayer << ": nG4hits = " << ng4hits[ilayer]
					<< " => nHits = " << nhits[ilayer] << " => nClusters = "
					<< nclusters[ilayer] << endl;
		}
		...
		cout << "nGtracks = "
				<< std::distance(truthinfo->GetPrimaryParticleRange().first,
		...
		if (verbosity > 1) {
			...
			for (PHG4TruthInfoContainer::ConstIterator iter = range.first;
				...
				cout << " pt = "
						<< sqrt(pow(particle->get_px(), 2) + pow(particle->get_py(), 2))
						<< endl;
				cout << " phi = " << atan2(particle->get_py(), particle->get_px())
						<< endl;
				cout << " eta = "
						<< asinh(
								particle->get_pz()
										/ sqrt(
												pow(particle->get_px(), 2)
														+ pow(particle->get_py(), 2))) << endl;

				cout << " embed flag = "
						<< truthinfo->isEmbeded(particle->get_track_id()) << endl;
				...
				for (std::set<PHG4Hit*>::iterator jter = g4hits.begin();
					...
					std::set<SvtxCluster*> clusters = clustereval->all_clusters_from(
							g4hit);
					for (std::set<SvtxCluster*>::iterator kter = clusters.begin();
							kter != clusters.end(); ++kter) {
				...
				if (trackmap && clustermap) {
					...
					for (std::set<SvtxTrack*>::iterator jter = tracks.begin();
						...
						cout << " nfromtruth = "
								<< trackeval->get_nclusters_contribution(track, particle)
								<< endl;

						cout
								<< " ---Associated-SvtxClusters-to-PHG4Hits-------------------------"
								<< endl;
...
void SvtxEvaluator::fillOutputNtuples(PHCompositeNode *topNode) {
	...
	if (verbosity > 0)
		cout << "SvtxEvaluator::fillOutputNtuples() entered" << endl;
	...
	if (hitmap_in) {
		for (SvtxHitMap::Iter iter = hitmap_in->begin(); iter != hitmap_in->end();
				++iter) {
			SvtxHit* hit = iter->second;
			float layer = hit->get_layer();
			if(layer >= _nlayers_maps + _nlayers_intt and layer <_nlayers_maps + _nlayers_intt + _nlayers_tpc)
				nhit_tpc_all++;
			if (layer == (float) _nlayers_maps + _nlayers_intt)
				nhit_tpc_in++;
			if (layer == (float) _nlayers_maps + _nlayers_intt + _nlayers_tpc - 1)
				nhit_tpc_out++;
			if (layer == (float) _nlayers_maps + _nlayers_intt + _nlayers_tpc / 2 - 1)
				nhit_tpc_mid++;
		}
	}
	...
	if (_ntp_vertex) {
		if (verbosity > 0) {
			...
			cout << "start vertex time:                "
					<< _timer->get_accumulated_time() / 1000. << " sec" << endl;
			_timer->restart();
		}
		...
		if (vertexmap && truthinfo) {
			...
			if (_do_eval_light == false) {
				for (auto iter = prange.first; iter != prange.second; ++iter) // process all primary paricle
					...
					if ((_scan_for_embedded && gembed <= 0) || (_do_eval_light == true))
						continue;

					std::set<SvtxCluster*> g4clusters = clustereval->all_clusters_from(
							g4particle);
					...
					for (const SvtxCluster* g4cluster : g4clusters) {
						...
						if (_nlayers_intt > 0 && layer >= _nlayers_maps
								&& layer < _nlayers_maps + _nlayers_intt) {
							lintt[layer - _nlayers_maps] = 1;
						}

						if (_nlayers_tpc > 0 && layer >= _nlayers_maps + _nlayers_intt
			...
			for (auto iter = vrange.first; iter != vrange.second; ++iter) // process all primary vertexes
				...
				if (search != embedvtxid_found.end()) {
					...
					if (vertex_particle_count[embedvtxid_vertex_id[gembed]]
							< vertex_particle_count[point_id]) {
						embedvtxid_vertex_id[gembed] = point_id;
						embedvtxid_vertex[gembed] = iter->second;
					}
			...
			for (std::map<int, bool>::iterator iter = embedvtxid_found.begin();
					iter != embedvtxid_found.end(); ++iter) {
				if (iter->first >= 0 || iter->first != iter->first)
					continue;
				++ngembed;
			}
			...
			for (SvtxVertexMap::Iter iter = vertexmap->begin();
				...
				if (point) {
					...
					if (embedvtxid_maps_particle_count[(int) gembed] > 0
							&& fabs(gvt) < 2000. && fabs(gvz) < 13.0
							&& (_do_eval_light == false))
				...
				float vertex_data[18] = { (float) _ievent, vx, vy, vz, ntracks, gvx,
						gvy, gvz, gvt, gembed, gntracks, gntracksmaps, gnembed, nfromtruth,
						nhit_tpc_all, nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
...
//        nfromtruth = vertexeval->get_ntracks_contribution(vertex,point);
					...
					float vertex_data[18] =
							{ (float) _ievent, vx, vy, vz, ntracks, gvx, gvy, gvz, gvt,
									gembed, gntracks, gntracksmaps, gnembed, nfromtruth,
									nhit_tpc_all, nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
		...
		if (truthinfo) {
			...
			for (auto iter = vrange.first; iter != vrange.second; ++iter) // process all primary vertexes
				...
				if (point) {
					...
					float gpoint_data[] = { (float) _ievent, gvx, gvy, gvz, gvt, gntracks,
							gembed, vx, vy, vz, ntracks, nfromtruth, nhit_tpc_all,
							nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
	...
	if (_ntp_g4hit) {
		...
		for (std::set<PHG4Hit*>::iterator iter = g4hits.begin();
			...
			float g4hit_data[50] = { (float) _ievent, g4hitID, gx, gy, gz, gt, gedep,
					geta, gphi, gdphi, gdz, glayer, gtrackID, gflavor, gpx, gpy, gpz, gvx,
					gvy, gvz, gfpx, gfpy, gfpz, gfx, gfy, gfz, gembed, gprimary,
					nclusters, clusID, x, y, z, eta, phi, e, adc, layer, size, phisize,
					zsize, efromtruth, dphitru, detatru, dztru, drtru, nhit_tpc_all,
					nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
	...
	if (_ntp_hit) {
		...
		SvtxHitMap* hitmap = findNode::getClass<SvtxHitMap>(topNode, "SvtxHitMap");
		PHG4CylinderCellGeomContainer* geom_container = findNode::getClass<
		...
		if (hitmap) {
			...
			for (SvtxHitMap::Iter iter = hitmap->begin(); iter != hitmap->end();
				...
				if (layer >= _nlayers_maps + _nlayers_intt) {
					PHG4CylinderCellGeom *GeoLayer = geom_container->GetLayerCellGeom(
							layer);
					//"cellID:ecell:phibin:zbin:phi:z"
				...
				float hit_data[40] = { event, hitID, e, adc, layer, cellID, ecell,
						(float) phibin, (float) zbin, phi, z, g4hitID, gedep, gx, gy, gz,
						gt, gtrackID, gflavor, gpx, gpy, gpz, gvx, gvy, gvz, gvt, gfpx,
						gfpy, gfpz, gfx, gfy, gfz, gembed, gprimary, efromtruth,
						nhit_tpc_all, nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
	...
	if (_ntp_cluster && !_scan_for_embedded) {
		...
		if (clustermap) {
			...
			for (SvtxClusterMap::Iter iter = clustermap->begin();
				...
				float cluster_data[50] = { (float) _ievent, hitID, x, y, z, r, phi, eta,
						ex, ey, ez, ephi, e, adc, layer, size, phisize, zsize, trackID,
						g4hitID, gx, gy, gz, gr, gphi, geta, gt, gtrackID, gflavor, gpx,
						gpy, gpz, gvx, gvy, gvz, gvt, gfpx, gfpy, gfpz, gfx, gfy, gfz,
						gembed, gprimary, efromtruth, nparticles, nhit_tpc_all, nhit_tpc_in,
						nhit_tpc_mid, nhit_tpc_out };
		...
		if (trackmap) {
			...
			for (SvtxTrackMap::Iter iter = trackmap->begin(); iter != trackmap->end();
				...
				for (SvtxTrack::ConstClusterIter iter = track->begin_clusters();
					...
					float cluster_data[46] = { (float) _ievent, hitID, x, y, z, r, phi,
							eta, ex, ey, ez, ephi, e, adc, layer, size, phisize, zsize,
							trackID, g4hitID, gx, gy, gz, gt, gtrackID, gflavor, gpx, gpy,
							gpz, gvx, gvy, gvz, gfpx, gfpy, gfpz, gfx, gfy, gfz, gembed,
							gprimary, efromtruth, nparticles, nhit_tpc_all, nhit_tpc_in,
							nhit_tpc_mid, nhit_tpc_out };
			...
			for (PHG4TruthInfoContainer::ConstIterator iter = range.first;
				...
				if (_do_track_match) {
					...
					if (track) {
						...
						nfromtruth = trackeval->get_nclusters_contribution(track,
								g4particle);
						nwrong = trackeval->get_nwrongclusters_contribution(track,
								g4particle);

						if (_nlayers_maps == 0) {
							ntrumaps = 0;
						} else {
							ntrumaps = trackeval->get_layer_range_contribution(track,
									g4particle, 0, _nlayers_maps);
						}
						if (_nlayers_intt == 0) {
							ntruintt = 0;
						} else {
							ntruintt = trackeval->get_layer_range_contribution(track,
									g4particle, _nlayers_maps, _nlayers_maps + _nlayers_intt);
						}
						ntrutpc = trackeval->get_layer_range_contribution(track, g4particle,
								_nlayers_maps + _nlayers_intt,
								_nlayers_maps + _nlayers_intt + _nlayers_tpc);

						layersfromtruth = trackeval->get_nclusters_contribution_by_layer(
								track, g4particle);
					}
				}
				float gtrack_data[66] = { (float) _ievent, gtrackID, gflavor, ng4hits,
						(float) ngmaps, (float) ngintt, (float) ngtpc, (float) nglmaps,
						(float) nglintt, (float) ngltpc, gpx, gpy, gpz, gpt, geta, gphi,
						gvx, gvy, gvz, gvt, gfpx, gfpy, gfpz, gfx, gfy, gfz, gembed,
						gprimary, trackID, px, py, pz, pt, eta, phi, charge, quality, chisq,
						ndf, nhits, (float) layers, nmaps, nintt, ntpc, nlmaps, nlintt,
						nltpc, dca2d, dca2dsigma, dca3dxy, dca3dxysigma, dca3dz,
						dca3dzsigma, pcax, pcay, pcaz, nfromtruth, nwrong, ntrumaps,
						ntruintt, ntrutpc, layersfromtruth, nhit_tpc_all, nhit_tpc_in,
						nhit_tpc_mid, nhit_tpc_out };
	...
	if (_ntp_track) {
		...
		if (trackmap) {
			...
			for (SvtxTrackMap::Iter iter = trackmap->begin(); iter != trackmap->end();
				...
				float track_data[82] = { (float) _ievent, trackID, px, py, pz, pt, eta,
						phi, charge, quality, chisq, ndf, nhits, nmaps, nintt, ntpc, nlmaps,
						nlintt, nltpc, (float) layers, dca2d, dca2dsigma, dca3dxy,
						dca3dxysigma, dca3dz, dca3dzsigma, pcax, pcay, pcaz, presdphi,
						presdeta, prese3x3, prese, cemcdphi, cemcdeta, cemce3x3, cemce,
						hcalindphi, hcalindeta, hcaline3x3, hcaline, hcaloutdphi,
						hcaloutdeta, hcaloute3x3, hcaloute, gtrackID, gflavor, ng4hits,
						(float) ngmaps, (float) ngintt, (float) ngtpc, (float) nglmaps,
						(float) nglintt, (float) ngltpc, gpx, gpy, gpz, gpt, geta, gphi,
						gvx, gvy, gvz, gvt, gfpx, gfpy, gfpz, gfx, gfy, gfz, gembed,
						gprimary, nfromtruth, nwrong, ntrumaps, ntruintt, ntrutpc,
						layersfromtruth, nhit_tpc_all, nhit_tpc_in, nhit_tpc_mid,
						nhit_tpc_out };
		...
		if (truthinfo) {
			...
			for (PHG4TruthInfoContainer::ConstIterator iter = range.first;
				...
				for (unsigned int i = 0;
						i < _nlayers_maps + _nlayers_intt + _nlayers_tpc; i++) {
					xval[i] = 0;
					yval[i] = 0;
					zval[i] = 0;
				}
				...
				for (unsigned int i = 0;
					...
					float gseed_data[30] = { (float) _ievent, ntrk, gx, gy, gz, gr, geta,
							gphi, glayer, gpx, gpy, gpz, gtpt, gtphi, gteta, gvx, gvy, gvz,
							gembed, gprimary, gflav, dphiprev, detaprev, nhit_tpc_all,
							nhit_tpc_in, nhit_tpc_mid, nhit_tpc_out };
