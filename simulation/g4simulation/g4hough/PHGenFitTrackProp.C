...
PHGenFitTrackProp::PHGenFitTrackProp(
		const string& name,
...
int PHGenFitTrackProp::Init(PHCompositeNode* topNode) {
	if(_analyzing_mode){
...
int PHGenFitTrackProp::InitRun(PHCompositeNode* topNode) {
...
int PHGenFitTrackProp::process_event(PHCompositeNode *topNode) {

  if (verbosity > 0){
	  cout << "PHGenFitTrackProp::process_event -- entered" << endl;
...
void PHGenFitTrackProp::print_timers() {
...
int PHGenFitTrackProp::End(PHCompositeNode *topNode) {
...
void PHGenFitTrackProp::projectToRadius(const SvtxTrack* track, double B,
...
void PHGenFitTrackProp::projectToRadius(const SvtxTrackState* state, int charge,
...
void PHGenFitTrackProp::set_material(int layer, float value) {
	_user_material[layer] = value;
}

int PHGenFitTrackProp::CreateNodes(PHCompositeNode* topNode) {
	// create nodes...
...
int PHGenFitTrackProp::InitializeGeometry(PHCompositeNode *topNode) {
...
int PHGenFitTrackProp::InitializePHGenFit(PHCompositeNode* topNode) {
...
int PHGenFitTrackProp::setup_seed_tracker_objects() {
...
int PHGenFitTrackProp::setup_initial_tracker_object() {
...
int PHGenFitTrackProp::setup_tracker_object() {
...
int PHGenFitTrackProp::GetNodes(PHCompositeNode* topNode) {
...
int PHGenFitTrackProp::translate_input() {
  ...
  if (verbosity > 20) {
    cout
      ...
      << endl;
    cout
      << "PHGenFitTrackProp::process_event has the following input clusters:"
      << endl;
...
int PHGenFitTrackProp::fast_vertex_from_bbc() {
...
int PHGenFitTrackProp::fast_vertex_guessing() {
...
int PHGenFitTrackProp::initial_vertex_finding() {
...
int PHGenFitTrackProp::vertexing(PHCompositeNode* topNode) {
...
//	_vertex[2] += _vertex_error[2] * gsl_ran_ugaussian(RandomGenerator);
	...
	if (verbosity > 1) {
		cout << __LINE__ << " PHGenFitTrackProp::vertexing: {" << _vertex[0]
				<< ", " << _vertex[1] << ", " << _vertex[2] << "} +- {"
...
int PHGenFitTrackProp::full_track_seeding() {
...
int PHGenFitTrackProp::export_output() {
	...
	if (verbosity > 0) {
		cout << "PHGenFitTrackProp::process_event -- leaving process_event"
				<< endl;
	}
...
int PHGenFitTrackProp::add_tracks() {
...
float PHGenFitTrackProp::kappaToPt(float kappa) {
	return _pt_rescale * _magField / 333.6 / kappa;
}

float PHGenFitTrackProp::ptToKappa(float pt) {
	return _pt_rescale * _magField / 333.6 / pt;
}

void PHGenFitTrackProp::convertHelixCovarianceToEuclideanCovariance(float B,
...
void PHGenFitTrackProp::shift_coordinate_system(double dx, double dy,
		double dz) {
...
bool PHGenFitTrackProp::circle_line_intersections(double x0, double y0,
...
int PHGenFitTrackProp::CleanupSeedsByHitPattern() {
...
int PHGenFitTrackProp::CleanupTracksByHitPattern() {
...
int PHGenFitTrackProp::check_track_exists(MapPHGenFitTrack::iterator iter){
...
int PHGenFitTrackProp::CleanupSeeds() {
...
int PHGenFitTrackProp::FullTrackFitting(PHCompositeNode* topNode) {

#ifdef _DEBUG_
	std::cout << "=========================" << std::endl;
	std::cout << "PHGenFitTrackProp::FullTrackFitting: Start: Event: "<< _event << std::endl;
...
#ifdef _DEBUG_
	std::cout << "=========================" << std::endl;
	std::cout << "PHGenFitTrackProp::FullTrackFitting: End: Event: "<< _event << std::endl;
...
//#ifdef _DEBUG_
//	std::cout << "=========================" << std::endl;
//	std::cout << "PHGenFitTrackProp::FullTrackFitting: Event: "<< _event << std::endl;
...
int PHGenFitTrackProp::SimpleTrack3DToPHGenFitTracks(PHCompositeNode* topNode, unsigned int itrack) {
	...
	if(nhits > 0 and chi2 > 0 and ndf > 0) {
		_PHGenFitTracks.push_back(
				MapPHGenFitTrack::value_type(
						PHGenFitTrackProp::TrackQuality(nhits, chi2, ndf, nhits, 0, 0), track)
		);
	}
	if(verbosity > 1) _t_translate3->stop();
	if(verbosity > 1){
...
int PHGenFitTrackProp::TrackPropPatRec(
...
//				);
		...
		PHGenFitTrackProp::TrackQuality tq(track_iter->first);
...
//								std::shared_ptr < PHGenFit::Track> (iter->second)));
				...
				_PHGenFitTracks.push_back(
						MapPHGenFitTrack::value_type(
								PHGenFitTrackProp::TrackQuality(
										tq.nhits + 1,
										tq.chi2  + iter->first,
										tq.ndf   + 2,
...
int PHGenFitTrackProp::BuildLayerZPhiHitMap() {
...
std::vector<unsigned int> PHGenFitTrackProp::SearchHitsNearBy(const unsigned int layer,
...
bool PHGenFitTrackProp::circle_circle_intersections(double x0, double y0,
