...
#ifndef __H_PHGenFitTrackProp_H__
#define __H_PHGenFitTrackProp_H__
...
///
class PHGenFitTrackProp: public SubsysReco {

public:

	PHGenFitTrackProp(
			const std::string &name = "PHGenFitTrackProp",
	...
	struct TrackQuality {
		...
		friend std::ostream& operator<<(std::ostream& os, const PHGenFitTrackProp::TrackQuality& tq) {
			os
			<< tq.nhits <<", "
			<< tq.chi2 <<", "<<tq.ndf <<", "
	...
	void setVoteErrorScale(unsigned int layer, float scale) {
		if (scale > 0.0) {
			...
			std::cout << "PHGenFitTrackProp::setVoteErrorScale : scale must be "
	...
	void setFitErrorScale(unsigned int layer, float scale) {
		if (scale > 0.0) {
			...
			std::cout << "PHGenFitTrackProp::setFitErrorScale : scale must be "
					"greater than zero ... doing nothing" << std::endl;
		}
	}
...
#endif // __H_PHGenFitTrackProp_H__
