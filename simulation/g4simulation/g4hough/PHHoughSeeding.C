...
int PHHoughSeeding::Init(PHCompositeNode* topNode) {
	if(_analyzing_mode){
...
int PHHoughSeeding::InitRun(PHCompositeNode* topNode) {
...
int PHHoughSeeding::process_event(PHCompositeNode *topNode) {
...
int PHHoughSeeding::End(PHCompositeNode *topNode) {
...
void PHHoughSeeding::projectToRadius(const SvtxTrack* track, double B,
...
void PHHoughSeeding::projectToRadius(const SvtxTrackState* state, int charge,
...
int PHHoughSeeding::vertexing(PHCompositeNode* topNode) {
...
bool PHHoughSeeding::circle_line_intersections(double x0, double y0,
...
int PHHoughSeeding::FullTrackFitting(PHCompositeNode* topNode) {

#ifdef _DEBUG_
	std::cout << "=========================" << std::endl;
	std::cout << "PHHoughSeeding::FullTrackFitting: Start: Event: "<< _event << std::endl;
...
#ifdef _DEBUG_
	std::cout << "=========================" << std::endl;
	std::cout << "PHHoughSeeding::FullTrackFitting: End: Event: "<< _event << std::endl;
...
int PHHoughSeeding::ExportOutput() { return 0;}

int PHHoughSeeding::OutputPHGenFitTrack(PHCompositeNode* topNode, MapPHGenFitTrack::iterator iter) {

//#ifdef _DEBUG_
//	std::cout << "=========================" << std::endl;
//	std::cout << "PHHoughSeeding::FullTrackFitting: Event: "<< _event << std::endl;
...
int PHHoughSeeding::SimpleTrack3DToPHGenFitTracks(PHCompositeNode* topNode, unsigned int itrack) {
	...
	<< ": PHHoughSeeding::SimpleTrack3DToPHGenFitTracks: "<< itrack
	...
	if(nhits > 0 and chi2 > 0 and ndf > 0) {
		_PHGenFitTracks.push_back(
				MapPHGenFitTrack::value_type(
						PHHoughSeeding::TrackQuality(nhits, chi2, ndf, nhits, 0, 0), track)
		);
	}
...
int PHHoughSeeding::TrackPropPatRec(
...
//				);
		...
		PHHoughSeeding::TrackQuality tq(track_iter->first);
...
PHGenFit::Measurement* PHHoughSeeding::SvtxClusterToPHGenFitMeasurement(
...
int PHHoughSeeding::BuildLayerZPhiHitMap() {
...
std::vector<unsigned int> PHHoughSeeding::SearchHitsNearBy(const unsigned int layer,
...
//std::shared_ptr<SvtxTrack> PHHoughSeeding::MakeSvtxTrack(
...
unsigned int PHHoughSeeding::encode_cluster_index(const unsigned int layer,
...
bool PHHoughSeeding::circle_circle_intersections(double x0, double y0,
