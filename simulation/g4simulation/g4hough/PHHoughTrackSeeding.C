...
PHHoughTrackSeeding::PHHoughTrackSeeding(
		const string& name,
...
int PHHoughTrackSeeding::Init(PHCompositeNode* topNode) {
	if(_analyzing_mode){
...
int PHHoughTrackSeeding::InitRun(PHCompositeNode* topNode) {
...
int PHHoughTrackSeeding::process_event(PHCompositeNode *topNode) {

  if (verbosity > 0){
	  cout << "PHHoughTrackSeeding::process_event -- entered" << endl;
...
void PHHoughTrackSeeding::print_timers() {
...
int PHHoughTrackSeeding::End(PHCompositeNode *topNode) {
...
void PHHoughTrackSeeding::projectToRadius(const SvtxTrack* track, double B,
...
void PHHoughTrackSeeding::projectToRadius(const SvtxTrackState* state, int charge,
...
void PHHoughTrackSeeding::set_material(int layer, float value) {
	_user_material[layer] = value;
}

int PHHoughTrackSeeding::CreateNodes(PHCompositeNode* topNode) {
	// create nodes...
...
int PHHoughTrackSeeding::InitializeGeometry(PHCompositeNode *topNode) {
...
int PHHoughTrackSeeding::setup_seed_tracker_objects() {
...
int PHHoughTrackSeeding::setup_initial_tracker_object() {
...
int PHHoughTrackSeeding::setup_tracker_object() {
...
int PHHoughTrackSeeding::GetNodes(PHCompositeNode* topNode) {
...
int PHHoughTrackSeeding::translate_input() {
  ...
  if (verbosity > 20) {
    cout
      ...
      << endl;
    cout
      << "PHHoughTrackSeeding::process_event has the following input clusters:"
      << endl;
...
int PHHoughTrackSeeding::fast_vertex_from_bbc() {
...
int PHHoughTrackSeeding::fast_vertex_guessing() {
...
int PHHoughTrackSeeding::initial_vertex_finding() {
...
int PHHoughTrackSeeding::vertexing(PHCompositeNode* topNode) {

	if(_g4vertexes->size()<=0) {
		cerr << PHWHERE << " ERROR: No valid vertex found" << endl;
		return Fun4AllReturnCodes::ABORTEVENT;
	}

	SvtxVertex *first_vertex = _g4vertexes->begin()->second;

	_vertex.clear();
	_vertex.assign(3, 0.0);

	_vertex[0] = first_vertex->get_x();
	_vertex[1] = first_vertex->get_y();
	_vertex[2] = first_vertex->get_z();

	_vertex_error[0] = sqrt(first_vertex->get_error(0, 0));
	_vertex_error[1] = sqrt(first_vertex->get_error(1, 1));
	_vertex_error[2] = sqrt(first_vertex->get_error(2, 2));

	if (verbosity > 1) {
		cout << __LINE__ << " PHHoughTrackSeeding::vertexing: {" << _vertex[0]
				<< ", " << _vertex[1] << ", " << _vertex[2] << "} +- {"
...
int PHHoughTrackSeeding::full_track_seeding() {
...
int PHHoughTrackSeeding::export_output() {
	...
	if (verbosity > 0) {
		cout << "PHHoughTrackSeeding::process_event -- leaving process_event"
				<< endl;
	}
...
int PHHoughTrackSeeding::add_tracks() {
...
float PHHoughTrackSeeding::kappaToPt(float kappa) {
	return _pt_rescale * _magField / 333.6 / kappa;
}

float PHHoughTrackSeeding::ptToKappa(float pt) {
	return _pt_rescale * _magField / 333.6 / pt;
}

void PHHoughTrackSeeding::convertHelixCovarianceToEuclideanCovariance(float B,
...
void PHHoughTrackSeeding::shift_coordinate_system(double dx, double dy,
		double dz) {
...
bool PHHoughTrackSeeding::circle_line_intersections(double x0, double y0,
...
int PHHoughTrackSeeding::CleanupSeedsByHitPattern() {
...
int PHHoughTrackSeeding::CleanupTracksByHitPattern() {
...
int PHHoughTrackSeeding::CleanupSeeds() {
...
bool PHHoughTrackSeeding::circle_circle_intersections(double x0, double y0,
