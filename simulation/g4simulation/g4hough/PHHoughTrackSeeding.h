/*!
 *  \file PHHoughTrackSeeding.h
 *  \brief Hough tracking using selected layers
 *  \detail modified from Alan Dion's HelixHough
 *  \author Haiwang Yu <yuhw@nmsu.edu>
 */

#ifndef __H_PHHoughTrackSeeding_H__
#define __H_PHHoughTrackSeeding_H__
...
///
class PHHoughTrackSeeding: public SubsysReco {

public:

	PHHoughTrackSeeding(
			const std::string &name = "PHHoughTrackSeeding",
	...
	struct TrackQuality {
		...
		friend std::ostream& operator<<(std::ostream& os, const PHHoughTrackSeeding::TrackQuality& tq) {
			os
			<< tq.nhits <<", "
			<< tq.chi2 <<", "<<tq.ndf <<", "
	...
	void setVoteErrorScale(unsigned int layer, float scale) {
		if (scale > 0.0) {
			...
			std::cout << "PHHoughTrackSeeding::setVoteErrorScale : scale must be "
	...
	void setFitErrorScale(unsigned int layer, float scale) {
		if (scale > 0.0) {
			...
			std::cout << "PHHoughTrackSeeding::setFitErrorScale : scale must be "
					"greater than zero ... doing nothing" << std::endl;
		}
	}
...
#endif // __H_PHHoughTrackSeeding_H__
